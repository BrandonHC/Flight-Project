...
void
printflSeats (flight_t * flight, int route)
{
  char seat = 'A';		//starting seat
  int i;

  for (i = 1;
       i <= sizeof (flight[0].seats[0]) / sizeof (flight[0].seats[0][0]); ++i)
    {
      if (i < 10)
  ...
  for (i = 0; i < FLIGHTROWSIZE; ++i)
    {
      for (int j = 0;
	   j < sizeof (flight[0].seats[0]) / sizeof (flight[0].seats[0][0]);
	   ++j)
	{
...
void
reserveSeats (flight_t * flight, customer_t * client, int userID, int route)
{
  ...
  while (availSeat == true)
      ...
      do
	{
	  ...
	  if (!
	      (cSeat <=
	       sizeof (flight[0].seats[0]) / sizeof (flight[0].seats[0][0])
	       && cSeat >= 1))
	...
	printf
	  ("This seat has been taken, please select a different seat.\n");
      else
	availSeat = false;
    }
...
bool
checkMaxFlights (flight_t * flight)
{
  int maxFlights = 0;
...
void
deleteUserSlot (flight_t * flight, customer_t * client, int userID)
{
  ...
  for (int i = 0; i < FLIGHTROWSIZE; ++i)
    for (int j = 0;
	 ...
	 ++j)
      {
	if (flight[client[userID - 1].routeNum].seats[i][j] ==
	    client[userID - 1].id)
...
int
accountCreate (customer_t * client, int *registry)
{
...
int
accountAuthenticate (customer_t * client)
{
  int userNum;
...
void
customerInterface (customer_t * client, flight_t * flight, int *registry)
{
  int uAccount = 0;
  ...
  while (uAccount != 3 && uAccount != -1)
      ...
      else if (uAccount == 2)
	{
	  ...
	  if (userID != -1)
		...
		{
		  printf
		    ("There is a flight registered with this account, would you like to delete your current reservation?\n[1] Delete\t[2] Exit\n");
		  do
		    {
		      scanf ("%d", &userChoice);
  ...
  if (uAccount == -1)
	...
	{
	  printf
	    ("All flights are at maximum capacity, no reservations can be made at this moment.\n");
	  return;
	}
      ...
      do
	{
	  ...
	  if (!(route == 1 || route == 2))
	    ...
	    printf
	      ("This flight has reached the max capacitants, please select a different flight: ");
	}
...
void
createEmployee (struct employee *point, int id, int fnum)
{
  point->empID = id;
  point->flight = fnum;
}
...
void
addEmployee (flight_t * flight, struct employee *employee, int fnum,
	     int crewnum)
{
...
void
removeEmployee (flight_t * flight, struct employee *employee, int fnum,
		int crewnum)
{
...
void
companyInterface (flight_t * flight, struct employee *employee,
		  Airport * NumberofAirports)
{
  int value;
  int RevAdd, NewLocation;

  printf
    ("Do you want to manage routes, manage employees, or exit the program?\n");
  printf
  ...
  switch (value)
    {
    case 1:
      printf ("you have selected to manage routes\n");
...
printf("")
      if (RevAdd == 2)
	{
      ...
      break;
    case 2:
      printf ("Enter [1] to Remove a plane, [2] to Add one.\n ");
      do
	{
	  scanf ("%d", &RevAdd);
	  if (RevAdd > 2 || RevAdd < 1)
      ...
      if (RevAdd == 1)
	{
	  ...
	  flight[RevAdd].AvailFlight = 0;
	  flight[RevAdd].PlaneNumber = flight[RevAdd].PlaneNumber + 1000;
	}
      if (RevAdd == 2)
	{
	  for (int i=0 i < 6; i++)
	    {
	      if (flight[i].AvailFlight == 0 && flight[i].PlaneNumber >= 6000)
		{
		  printf ("ID %d, Plane %d\n\n", i, flight[i].PlaneNumber);
		}
	    }
	  printf
	    ("Enter the ID number of the place(Not the Plane Number) to Add a plane.\n");
	  do
	    {
	      scanf ("%d", &RevAdd);
	      if (RevAdd > 6 || RevAdd < 0)
    ...
    case 3:
      printf ("Enter [1] to add an airport,[2] to remove one.\n");
      do
	{
	  scanf ("%d", &RevAdd);
	  if (RevAdd > 2 || RevAdd < 1)
    ...
    case 4:
      printf ("you have selected to manage employees\n");
      ...
      else
	{
      ...
      break;
    default:
      printf ("You have selected to return to the menu\n");
    }
}

void
initializeFlights (flight_t * flight, int flightNum)
{
  for (int i = 0; i < FLIGHTROWSIZE; ++i)
    {
      for (int j = 0;
...
//Identify Flight routes
  ...
  strcpy (flight[2].OutPort, "Corpus");
  ...
  flight[3].flightnumber = 5;
  flight[3].PlaneNumber = 6005;
  flight[3].PriceofFlight = 150.25;
  strcpy (flight[3].InAirport, "SanAntonio");
  flight[3].InAirportCode = 1;
  strcpy (flight[3].OutPort, "Corpus");
  flight[3].OutAirportCode = 3;
  flight[3].distance = 150;
  flight[3].AvailFlight = 0;
